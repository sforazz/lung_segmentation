...
#             outname = cluster_correction(outname, 0.2, 10000)
    ...
    def run_evaluation(self):
        ...
        LOGGER.info('Standard Deviation DSC: %s', np.std(all_dsc))
        LOGGER.info('Median HD: %s', np.median(all_hd))
        LOGGER.info('Standard Deviation HD: %s', np.std(all_hd))
        LOGGER.info('Max DSC: %s', np.max(all_dsc))
        LOGGER.info('Min DSC: %f', np.min(all_dsc))
        LOGGER.info('Max HD: %f', np.max(all_hd))
        LOGGER.info('Min HD: %f', np.min(all_hd))
